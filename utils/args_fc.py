...
def eval_args():
    ...
    parser.add_argument("--seed", type=int, dest="seed", required=True, help="Seed to use")
    parser.add_argument("--dataset", type=str, default=default_dataset, dest="dataset", help="dataset to train on",)
    parser.add_argument("--ood_dataset", type=str, default=ood_dataset, dest="ood_dataset", help="OOD dataset for given training dataset",)
    parser.add_argument("--data-aug", action="store_true", dest="data_aug")
...
def al_args():
    ...
    parser.add_argument("--seed", type=int, dest="seed", required=True, help="Seed to use")
    parser.add_argument("--model", type=str, default=model_name, dest="model_name", help="Model to train",)
    parser.add_argument("-ambiguous", action="store_true", dest="ambiguous", help="Use Ambiguous MNIST in training",)
    ...
    parser.add_argument("--trained-model", type=str, default=trained_model_name, dest="trained_model_name", help="Trained model to check entropy of acquired samples",)

    parser.add_argument("-tsn", action="store_true", dest="tsn", help="whether to use spectral normalisation",)
    parser.set_defaults(tsn=False)
    parser.add_argument("--tcoeff", type=float, default=sn_coeff, dest="tcoeff", help="Coeff parameter for spectral normalisation",)
    parser.add_argument("-tmod", action="store_true", dest="tmod", help="whether to use architectural modifications during training",)
    parser.set_defaults(tmod=False)

    parser.add_argument("--saved-model-path", type=str, default=saved_model_path, dest="saved_model_path", help="Path of pretrained model",)
    parser.add_argument("--saved-model-name", type=str, default=saved_model_name, dest="saved_model_name", help="File name of pretrained model",)
    parser.add_argument("--threshold", type=float, default=threshold, dest="threshold", help="Entropy threshold to decide if a sample is ambiguous or not",)
    parser.add_argument("--subsample", type=int, default=subsample, dest="subsample", help="Subsample for MNIST",)

    parser.add_argument("--num-ensemble", type=int, default=num_ensemble, dest="num_ensemble", help="Number of models in ensemble",)
    ...
    parser.add_argument("--al-type", type=str, default=al_acquisition, choices=["softmax", "ensemble", "gmm"], dest="al_type", help="Type of model to use for AL.",)

    parser.add_argument("-mi", action="store_true", dest="mi", help="Use MI as acquisition function")
    parser.set_defaults(mi=False)

    parser.add_argument("--num-initial-samples", type=int, default=num_initial_samples, dest="num_initial_samples", help="Initial number of samples in the training set",)
    parser.add_argument("--max-training-samples", type=int, default=max_training_samples, dest="max_training_samples", help="Maximum training set size",)
    parser.add_argument("--acquisition-batch-size", type=int, default=acquisition_batch_size, dest="acquisition_batch_size", help="Number of samples to acquire in each acquisition step",)

    parser.add_argument("-e", type=int, default=epochs, dest="epochs", help="Number of epochs to train after each acquisition",)
    parser.add_argument("-b", type=int, default=train_batch_size, dest="train_batch_size", help="Training batch size",)
    parser.add_argument("-tb", type=int, default=test_batch_size, dest="test_batch_size", help="Test batch size",)
    parser.add_argument("-sb", type=int, default=scoring_batch_size, dest="scoring_batch_size", help="Batch size for scoring the pool dataset",)
